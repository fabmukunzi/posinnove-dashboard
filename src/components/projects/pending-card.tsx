...
function PendingCard({ project }: { project: any }) {
  ...
  const completedPercentage = Math.floor(
    (project.status.completed / totalTasks) * 100
  );
  const underReviewPercentage = Math.floor(
    (project.status.underReview / totalTasks) * 100
  );
  const onProgressPercentage = Math.floor(
    (project.status.onprogress / totalTasks) * 100
  );
  const backlogPercentage = Math.floor(
    (project.status.backlog / totalTasks) * 100
  );
  ...
  return (
    <div className="border w-full border-borderColor rounded-xl bg-surface p-4">
      <div className="leading-2">
        <div className="p-5">
          <div>
            <h1 className="text-lg font-bold overflow-hidden whitespace-nowrap  text-ellipsis">
              {project.title}
            </h1>
          ...
          <div className="flex flex-col gap-2 text-[100%]">
            ...
            <div className="flex justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold md:text-[90%]">Task Done</p>
              </div>
            ...
            <div className="flex justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold md:text-[90%]">Under Review</p>
              </div>
            ...
            <div className="flex justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold md:text-[90%]">On-Progress</p>
              </div>
            ...
            <div className="flex justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold md:text-[90%]">Backlog</p>
              </div>
        ...
        <div className="mx-5 my-2 w-[80%]">
          ...
          <div className="bg-zinc-300 h-2 rounded-full flex overflow-hidden">
            <div
              className="bg-[#1375F6] rounded-full"
              style={{ width: `${completedPercentage}%` }}
            ></div>
            <div
              className="bg-[#EEB765] rounded-r-full"
              style={{ width: `${underReviewPercentage}%` }}
            ></div>
            <div
              className="bg-[#336940] rounded-r-full"
              style={{ width: `${onProgressPercentage}%` }}
            ></div>
            <div
              className="bg-[#D9D9D9] rounded-r-full"
        ...
        <div className="bg-[#c7dcf07b] w-full">
          <p className="text-lg w-full px-5 py-2">
            <span className="font-bold text-lg">Next Task:</span>{" "}
        ...
        <div className="mx-5 flex justify-between items-center px-5 py-2">
          <div className="flex flex-wrap w-full gap-4 items-center">
            ...
            <Button className="rounded-2xl w-[100%] text-[75%] px-2">
              Go to Project 
            </Button>
          </div>
        </div>
